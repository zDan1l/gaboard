...
@section('title', 'Input Nilai Kepuasan Pelanggan - GaBoard')
...
<div class="space-y-6">
    ...
    <div class="bg-white shadow-sm rounded-lg p-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Input Nilai Kepuasan Pelanggan</h1>
                <p class="text-sm text-gray-600 mt-1">Beri penilaian untuk karyawan berdasarkan pelayanan kepada pelanggan</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white shadow-sm rounded-lg overflow-hidden">
        <form method="POST" action="{{ route('customer-satisfaction.store') }}" class="p-6 space-y-6" id="scoreForm">
            @csrf
            ...
            <div>
                ...
                <div class="flex items-center space-x-4">
                    <input type="range" name="score" id="scoreInput" min="1" max="5" step="0.1" value="3" class="flex-1 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" oninput="updateScorePreview()">
                    <span id="scorePreview" class="text-2xl font-bold text-orange-600 w-12 text-center">3.0</span>
                </div>
            ...
            <div>
                ...
                <input type="month" name="period" id="periodInput" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-orange-500 focus:border-orange-500">
            ...
            <div>
                ...
                <textarea name="notes" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-orange-500 focus:border-orange-500" placeholder="Tambahkan catatan atau alasan penilaian...">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
...
document.getElementById('periodInput').value = `${now.getFullYear()}-${month}`;
</script>
@endpush
@endsection
